...
class Model:
    ...
    def train(self, matrix,returns, old_ps, old_value,hidden_state):
        ...
        with autocast():
            new_ps, new_v,  _ = self.network(matrix, hidden_state)
            ratio = torch.exp(torch.log(torch.clamp(new_ps, 1e-6, 1.0)) - torch.log(torch.clamp(old_ps, 1e-6, 1.0)))
            entropy = torch.mean(-torch.sum(new_ps * torch.log(torch.clamp(new_ps, 1e-6, 1.0)), dim=-1, keepdim=True))
            
            # critic loss
            new_v = torch.squeeze(new_v)
            new_v_clipped = old_value + torch.clamp(new_v - old_value, - training_parameter.clip_range, training_parameter.clip_range)
            ...
            policy_losses2 = advantage * torch.clamp(ratio, 1.0 - training_parameter.clip_range, 1.0 + training_parameter.clip_range)
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(self.network.parameters(), training_parameter.max_grad_norm)
        ...
        stats_list = [all_loss.cpu().detach().numpy(), policy_loss.cpu().detach().numpy(),
                      ...
                      critic_loss.cpu().detach().numpy(),grad_norm.cpu().detach().numpy(),
