...
class Model:
    def __init__(self,global_model = False):
        ...
        # only train global model and need optimizer
        if global_model:
    ...
    def step(self,obs,hidden_state):
        ...
        v = obs['v']
        box = obs['box']
        view.resize(1,custom_parameter.render_height,custom_parameter.render_width,training_parameter.channel)
        view = torch.from_numpy(view).to(self.device)
        v = torch.from_numpy(v).to(self.device)
        box = torch.from_numpy(box).to(self.device)
        # run forward process
        policy,value,hidden_state = self.net(view,v,box,hidden_state)
        policy = np.squeeze(policy.cpu().detach().numpy())
        value = value.cpu().detach().numpy()
        return policy,value,hidden_state
    
    def train(self, obs,returns, old_ps, old_value, hidden_state):
        # convert slice data to tensor
        obs_tensors = []
        for item in obs:
            tensor_dict = {}
            for key, value in item.items():
                tensor_dict[key] = torch.from_numpy(value).to(self.device)
            obs_tensors.append(tensor_dict)    
        ...
        with autocast():
            for i in range(old_ps.shape[0]):
                # gain new policy and value with model
                new_ps, new_v,  _ = self.net(obs_tensors[i]['view'],obs_tensors[i]['v'],obs_tensors[i]['box'], hidden_state[i])
